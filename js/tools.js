...
var earthToolWidth = Math.ceil(60 * zoomInit + 32); //宽
...
function getTableObjectById(id) {
    ...
    if (menuCon.item === undefined) {
        $("#div_toolBar").css({"background": "#2A80C9"});
        $("#div_PreNext").css({"background": "#2A80C9"});
    } else {
        $("#div_toolBar").css({"background": "white"});
        $("#div_PreNext").css({"background": "white"});
    }
    if (isHtmlExist) {
        $("#menu" + id).show();
        setButtonStatus();
    ...
    setToolsIconStatus();
    if (menuCon.item === undefined) {
        // clickItem(menuCon.id, $("#earthTools"))
        // //点击工具栏隐藏查询页面
        // hideRouteDetails();
        $('#' + top.topDialogId).click();
    }
    if (id == 6) {
        top.show3DMap();
    } else if (id == 1) {
        if (top.dialogId !== 'PatrolMonitor' && top.dialogId !== 'VisitMonitor') {
            // clickItem(top.topDialogId, $("#earthTools"))
            // //点击工具栏隐藏查询页面
            // hideRouteDetails();
            $('#' + top.topDialogId).click();
        }
