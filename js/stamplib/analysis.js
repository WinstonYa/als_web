 ...
 //        }
	...
	var lineOfSight = function(startHeight, endHeight, btn) {
        ...
        earth.Event.OnCreateGeometry = function(p, cType) {
            if (p.Count < 2) {
            	...
            	lineArr.push(p);
	            earth.ShapeCreator.Clear();
	            earth.Analysis.LineSight(3, parseFloat(startHeight), parseFloat(endHeight), lineArr[0], 3); //最后参数：1代表只进行服务端分析，2代表只进行本地分析，3代表二者都参与
            }
        };
        earth.ShapeCreator.CreateLine();
		earth.focus();
	...
	var showMoveHtml = function(anaLysisChk) {
		...
		earth.Event.OnDocumentReadyCompleted = function(guid) {
			...
			earth.Tools = Tools;
			earth.exitSkyLineAnalysis = exitSkyLineAnalysis;
			try {
	...
	function exitSkyLineAnalysis(){
        top.$('#MapTwo').trigger('mousedown', ['right']);
	}
