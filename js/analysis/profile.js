...
function getEarth(earthObj) {
	...
	function OnProfile(p, cType) {
		...
		g_polyline = seearth.Factory.CreateElementLine(seearth.Factory.CreateGUID(), "剖面线");
	    g_polyline.BeginUpdate();
	    g_polyline.SetPointArray(p);
	    g_polyline.LineStyle.LineWidth = 1;
	    g_polyline.LineStyle.LineColor = 0xffFFFF00;
	    g_polyline.AltitudeType = 1;
	    g_polyline.Selectable = false;
	    g_polyline.Editable = false;
	    g_polyline.EndUpdate();
	    seearth.AttachObject(g_polyline);
	...
	function clearResult() {
		...
		if(g_polyline){
			seearth.DetachObject(g_polyline);
			g_polyline = null;
		}
		seearth.ShapeCreator.Clear();
	}
