...
function initSelectTree(id, isMultiple, chkboxType,datas) {
    var setting = {
        view: {
            dblClickExpand: true,
            showLine: true
    ...
    var html = '<div class = "layui-select-title" >' +
        '<input id="' + id + 'Show"' + 'type = "text" placeholder = "请选择部门" value = "" class = "layui-input" readonly>' +
    ...
    $("#" + id).parent().append('<div class="tree-content scrollbar">' +
        ...
        '</div>');
    // $(".tree-content").css('width', $("#" + id).css('width'));//
    $(".tree-content").hide(); $(".tree-content input").hide();
    ...
    $("#" + id).bind("click", function () {
        if ($(this).parent().find(".tree-content").css("display") !== "none") {
            ...
            var width = $(this).css('width'); // var width = $(this).width() - 2;
    ...
    $.fn.zTree.init($("#" + id + "Tree"), setting, datas);
    var ztree = $.fn.zTree.getZTreeObj(id + "Tree");
...
function assignment(treeId, nodes) {
    ...
    $("#" + treeId + "Show").attr("title", names);$("#" + treeId + "Show").show();
