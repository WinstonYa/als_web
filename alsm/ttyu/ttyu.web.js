...
//var serverJson = {isJsonp:true,url:'http://localhost:50471/plat.aspx',serverUrl: 'http://219.140.191.226:8080/ttyuJiali/'}
var serverJson = {isJsonp:true,url:'http://ttyu.net/p/plat.aspx',serverUrl: 'http://ttyu.net:8080/patrol/'}
//用户信息
var user = { userID: 1, userName: 'admin', headImage: '',projectID: 1,deptID: null };
var vm;
var apps = [], appTree;
...
function init() {
    ...
    ttyu = {        
        ...
        getUrlKeyValue: function (key) {//获使url的键值
            var url = document.location.search.trimEnd("?").trimStart("?");
            if(url.contains("?"))url=url.getBefore("?");
            if(url=="")return null;
            return url.getUrlValue(key);
        },
        getKey: function (key) {//获使url的键值
        ...
        loadData: function (me, nodes, row) {
            ...
            try {
                ...
                if (row.attr.clickNode != null) {
                    ...
                    if(node==null&&row.ztree.getNodes().length>0)
                       node= row.ztree.getNodes()[0].children[row.attr.clickNode];
                }else{
...
//                        }
    ...
    ttyu.server = {
        ...
        doApiNet: function(dataJSON,func) {//跨域调用远程服务url,执行func           
            dataJSON.projectId = user.projectID;          
            dataJSON.userId = user.userID;
            dataJSON.compId = user.compID;      
            dataJSON = JSON.stringify(dataJSON);
            $.ajax({
                url: serverJson.url, type: "post",jsonp: "callback",async:false,
                data: { "datas": dataJSON },
                success: function (datas) {
                    if(typeof(datas)=="string")datas=ttyu.json.toJson(datas);
                    ttyu.server.successData("", datas, "text", func);                   
        ...
        doSysApi: function (name,dataJSON, func) {//执行Action
            var url=getServerUrl()+name;//url="http://localhost:8080/platProj/dropMenuProj"
...
//            var base64 = base64.getBetween('url("', ")");
        ...
        addFile: function (row, fileName, noDel) {//追加文件
            ...
            if (row.allowed != null) {//accept="image/gif, image/x-ms-bmp, image/bmp" application/msexcel  application/msword  application/pdf
                ...
                $.each(row.allowed, function (index, value) {
                    type="image";
                    if(value=="html"||value=="htm"||value=="css"||value=="csv"||value=="js"||value=="txt")
                       type= "text";
                    if(value=="3gpp"||value=="ac3"||value=="au"||value=="mp3"||value=="mp4"||value=="aid")
                       type= "audio";
                    if(value=="mpg"||value=="mpeg")
                       video= "text";
                    if(value=="doc"||value=="dot"||value=="dtd"||value=="csv"||value=="json"||value=="mpp"||value=="mpp"||value=="ogg"||value=="mpp"||value=="pdf"||value=="pot"||value=="pps"||value=="ppt"||value=="rtf"||value=="wdb"||value=="wps"||value=="xhtml"||value=="xlc"||value=="xlm"||value=="xls"||value=="xlt"||value=="xlw"||value=="zip")
                       type= "application";
                    accepts += type + value + ",";
                });
        ...
        init: function (row) {
            row.me=$(row.el);if(!row.me.hasClass('tt-file'))row.me.addClass('tt-file');
            row.el=row.el.replace("#","");
        ...
        close: function (that) {//
            that = $(that); var div = that.parent();
            var row =findOne(apps,'el',div.parent().attr('id'));
            var value=row.value;div.remove();
            var v = value.replace(value, '').trimEnd(",").trimStart(",").replace(',,', ',');
            row.value=v; 
//            var filename = div.css('background-image').getAfter('uploads/').getBefore(')').trimEnd("\"");
//            //ttyu.sql.delFiles(filename); //删除文件
//            var divName = that.parent().parent();
//            var v = divName.attr('value').replace(filename, '').trimEnd(",").trimStart(",").replace(',,', ',');
//            divName.attr('value', v); 
            //var row = page[that.parent().parent().attr('name')].row;
        },
...
///轮播
    function initLunbobox(ui) {
        var id = ui.attr('id');
        var toright = ui.find('.toright'); var toleft = ui.find('.toleft');
        toright.hide(); toleft.hide();
